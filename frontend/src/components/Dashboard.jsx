...
export function Dashboard({ setView, historyItems = HISTORY, reportsData = null, backendStatus = {} }) {
  ...
  const isFirstUse = totalScans === 0 && !backendStatus?.connected;

  return (
    <div className="view">

      {/* ── Onboarding Hero (first-time or disconnected) ── */}
      {isFirstUse && (
        <div style={{
          background: "linear-gradient(135deg, rgba(0,212,255,.06) 0%, rgba(0,255,136,.04) 50%, rgba(139,92,246,.05) 100%)",
          border: "1px solid rgba(0,212,255,.18)", borderRadius: 12, padding: "28px 28px",
          marginBottom: 20, position: "relative", overflow: "hidden"
        }}>
          <div style={{ position: "absolute", inset: 0, backgroundImage: "radial-gradient(circle at 80% 20%, rgba(0,255,136,.08) 0%, transparent 60%)", pointerEvents: "none" }} />
          <div className="scan-beam" />
          <div style={{ display: "flex", alignItems: "center", gap: 20, position: "relative" }}>
            <div>
              <div style={{ fontSize: 22, fontWeight: 900, marginBottom: 5, background: "linear-gradient(130deg, var(--cyan), var(--green))", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>
                Welcome to GhostTrace
              </div>
              <div style={{ fontFamily: "var(--mono)", fontSize: 11, color: "var(--t2)", marginBottom: 16, lineHeight: 1.7 }}>
                Your AI-powered cybersecurity intelligence platform. Connect your API keys in Settings, then run your first scan to begin building your threat intelligence picture.
              </div>
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                <button className="btn btn-primary btn-sm" onClick={() => setView("file-scan")}>🔍 Scan a File</button>
                <button className="btn btn-sec btn-sm" onClick={() => setView("url-scan")}>🌐 Analyze URL</button>
                <button className="btn btn-ghost btn-sm" onClick={() => setView("settings")}>⚙️ Configure APIs</button>
                <button className="btn btn-ghost btn-sm" onClick={() => setView("about")}>ℹ️ Learn More</button>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="card">
        <div className="card-hd">
          ...
          <span className="fac gap6 mono txt-xs txt-muted">
            ...
            {liveFeed.length ? "LIVE · Updated from monitor alerts" : "LIVE · Derived from latest backend scan history"}
          </span>
        </div>
        <div style={{ padding: 0 }}>
          {feedRows.length === 0 ? (
